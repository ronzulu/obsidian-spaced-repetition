...
export class NoteParser {
    ...
    async parse(noteFile: ISRFile, folderTopicPath: TopicPath, defaultTextDirection: TextDirection): Promise<Note> {
        const questionParser: NoteQuestionParser = new NoteQuestionParser(this.settings);
        const questions = await questionParser.createQuestionList(noteFile, folderTopicPath, true, defaultTextDirection);
