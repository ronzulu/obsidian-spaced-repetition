...
<!--SR:!2023-09-02,4,270-->`,
    ...
    expect(actual).toEqual([
        RepItemScheduleInfo_Simple.fromDueDateStr("2023-09-02", 4, 270, -4 * TICKS_PER_DAY),
    ]);
});
...
test("Multiple schedule info for question (on separate line)", () => {
    ...
    expect(actual).toEqual([
        RepItemScheduleInfo_Simple.fromDueDateStr("2023-09-03", 1, 230, -3 * TICKS_PER_DAY),
        RepItemScheduleInfo_Simple.fromDueDateStr("2023-09-05", 3, 250, -1 * TICKS_PER_DAY),
        RepItemScheduleInfo_Simple.fromDueDateStr("2023-09-06", 4, 270, 0),
    ]);
});
